 ...
 * @author Rado
 */
public class TraitementMontant extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet TraitementMontant</title>");            
            out.println("</head>");
            out.println("<body>");
            try {
                ...
                if(request.getParameter("date1")!=null && request.getParameter("date2")!=null) {
                    ...
                    double[] modeP = p.getModePaiement(d1, d2);
                    request.setAttribute("modeP", modeP);
                    double somme = 0;
                    if(modeP.length>0) {
                        somme = p.somme(modeP);
                    } 
                    ...
                    Paiement p = new Paiement();
                    double[] modeP = p.getModePaiementII();
                    request.setAttribute("modeP", modeP);
                    double somme = 0;
                    if(modeP.length>0) {
                        somme = p.somme(modeP);
                ...
                RequestDispatcher dispatch = request.getRequestDispatcher("ListeParPaiement.jsp");
